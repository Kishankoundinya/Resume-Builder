  ...
  return (
    <div className='space-y-4'>
        ...
        <div className='flex gap-2'>
            <input placeholder='Enter a skill (e.g.,JavaScript Project Management)' className='flex-1 px-3 py-2 text-sm' type="text" onChange={(e)=>setNewSkills(e.target.value)} value={newSkills} onKeyDown={handelkeyPress} />
            <button onClick={addSkills} className='flex items-center gap-2 px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed'><Plus className='size-4'/>Add</button>
        </div>
        {data.length >0 ?(
